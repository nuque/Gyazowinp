...
BOOL				saveLocal(LPCTSTR fileName);
...
// PNG ファイルをコピーする
BOOL saveLocal(LPCTSTR fileName){
	if(g_Settings.count(L"save_local")){
		WCHAR buffer[MAX_PATH];
		ExpandEnvironmentStrings(g_Settings[L"save_local"].c_str(),buffer,sizeof( buffer ));
		WCHAR dt[32]; // 日付
		GetDateFormat(LOCALE_USER_DEFAULT, 0, NULL, L"yyyyMMdd", dt, sizeof(dt));
		wcscat(buffer,dt);
		WCHAR tm[32]; // 時刻
		GetTimeFormat(LOCALE_USER_DEFAULT, 0, NULL, L"HHmmss", tm, sizeof(tm));
		wcscat(buffer,tm);
		LPCWSTR localPath = wcscat(buffer,L".png");
		if(!CopyFile(fileName, localPath, FALSE)){
			return FALSE;
		}
	}
}
...
//	const TCHAR* UPLOAD_PATH	= _T("/upload.cgi");
	...
	// アップロードしない
	if (g_Settings.count(L"no_upload") && g_Settings[L"no_upload"] == L"yes") {
		return TRUE;
	}
